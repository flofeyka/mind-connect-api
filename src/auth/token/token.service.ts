...
export class TokenService {
  ...
  async saveToken(refreshToken: string): Promise<string> {
    ...
    const tokenExisted: Token | null = await this.tokenRepository.findOneBy({
      user: { id: tokenVerifiedData.id },
    });
    if (!tokenExisted) {
      const insertTokenResult: InsertResult = await this.tokenRepository
        .createQueryBuilder()
        .insert()
        .into(Token)
        .values({ token: refreshToken, user: { id: tokenVerifiedData.id } })
        .execute();
      const tokenCreated: Token = await this.tokenRepository.findOneBy({
        id: insertTokenResult.identifiers[0].id,
      });
      return tokenCreated.token;
    }

    tokenExisted.token = refreshToken;
    const tokenUpdateResult: Token =
      await this.tokenRepository.save(tokenExisted);
  ...
  async verifyAccessToken(
    ...
    const verified: { id: number; email: string } = this.jwtService.verify(
      accessToken,
      {
        secret: process.env.JWT_ACCESS_SECRET,
      },
  ...
  verifyRefreshToken(refreshToken: string): { id: number; email: string } {
  ...
  async generateResetPasswordToken(user: User): Promise<ResetPasswordToken> {
    const tokenExists: ResetPasswordToken | null =
      await this.resetTokenRepository.findOneBy({ user });
    const FIVE_MINUTES: number = 5 * 60 * 1000;
    if (
      tokenExists &&
      Date.now() - new Date(tokenExists.updatedAt).getTime() < FIVE_MINUTES
    ) {
